...
class GC_TOC_Manager {
    ...
    public function add_ids_to_headings($content) {
        ...
        foreach ($matches[0] as $key => $heading) {
            ...
            $title_without_emoji = preg_replace('/[^\x00-\x7F]+|[\x{1F600}-\x{1F64F}\x{1F300}-\x{1F5FF}\x{1F680}-\x{1F6FF}\x{2600}-\x{26FF}\x{2700}-\x{27BF}]+/u', '', $title);

            if ($count > 0) {
                $id = sanitize_title($title_without_emoji) . '-' . ($count + 1);
            } else {
                $id = sanitize_title($title_without_emoji);
            }
